...
import {
  getPreferredServiceProviderById,
  updatePreferredServiceProviderById,
} from 'apiSdk/preferred-service-providers';
...
function PreferredServiceProviderEditPage() {
  ...
  const handleSubmit = async (values: PreferredServiceProviderInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePreferredServiceProviderById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Preferred Service Provider',
              isCurrent: true,
            },
          ]}
        />
      ...
      <Box rounded="md">
        <Box mb={4}>
          <Text as="h1" fontSize={{ base: '1.5rem', md: '1.875rem' }} fontWeight="bold" color="base.content">
            Update Preferred Service Provider
          </Text>
        </Box>
        {(error || formError) && (
          <Box mb={4}>
...
)(PreferredServiceProviderEditPage);
