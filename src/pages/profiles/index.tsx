...
import { getProfiles, deleteProfileById } from 'apiSdk/profiles';
import { ProfileInterface } from 'interfaces/profile';

type ColumnType = ColumnDef<ProfileInterface, unknown>;
...
export function ProfileListPage(props: ProfileListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getProfiles({
        ...
        order: params.order,
        searchTermKeys: ['bio.contains', 'location.contains', 'website.contains', 'social_links.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProfileInterface>>(
    () => `/profiles?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ProfileInterface) => {
    if (hasAccess('profile', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/profiles/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'bio', header: 'Bio', accessorKey: 'bio' },
    { id: 'location', header: 'Location', accessorKey: 'location' },
    { id: 'website', header: 'Website', accessorKey: 'website' },
    { id: 'social_links', header: 'Social Links', accessorKey: 'social_links' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/profiles/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('profile', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/profiles/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('profile', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="profile" />
        </Flex>

        {hasAccess('profile', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/profiles/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
