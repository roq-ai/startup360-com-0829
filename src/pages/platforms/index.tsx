...
import { getPlatforms, deletePlatformById } from 'apiSdk/platforms';
import { PlatformInterface } from 'interfaces/platform';

type ColumnType = ColumnDef<PlatformInterface, unknown>;

interface PlatformListPageProps {
...
export function PlatformListPage(props: PlatformListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getPlatforms({
        relations: ['user', 'service_provider.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PlatformInterface>>(
    () => `/platforms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PlatformInterface) => {
    if (hasAccess('platform', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/platforms/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('service_provider', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.service_provider,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/platforms/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('platform', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/platforms/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('platform', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
