...
import { getPreferredServiceProviders } from 'apiSdk/preferred-service-providers';
...
import { AccountListPage as ShowCaseList } from 'pages/accounts';

function DashboardPage() {
  const { session } = useSession();

  const preferredServiceProviderStat = useSWR<PaginatedInterface<PreferredServiceProviderInterface>>(
    () => `/preferred-service-providers`,
    () => getPreferredServiceProviders({ limit: 1, offset: 0 }),
  );

  const accountStat = useSWR<PaginatedInterface<AccountInterface>>(
    () => `/accounts`,
    () => getAccounts({ limit: 1, offset: 0 }),
  );

  const serviceProviderStat = useSWR<PaginatedInterface<ServiceProviderInterface>>(
    () => `/service-providers`,
    () => getServiceProviders({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Preferred Service Providers',
      result: preferredServiceProviderStat?.data?.totalCount,
      isLoading: preferredServiceProviderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Accounts',
      result: accountStat?.data?.totalCount,
      isLoading: accountStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Service Providers',
      result: serviceProviderStat?.data?.totalCount,
      isLoading: serviceProviderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
