...
import { getAccounts, deleteAccountById } from 'apiSdk/accounts';
import { AccountInterface } from 'interfaces/account';

type ColumnType = ColumnDef<AccountInterface, unknown>;
...
export function AccountListPage(props: AccountListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getAccounts({
        ...
        order: params.order,
        searchTermKeys: ['username.contains', 'email.contains', 'password.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AccountInterface>>(
    () => `/accounts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AccountInterface) => {
    if (hasAccess('account', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/accounts/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/accounts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('account', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/accounts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('account', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="account" />
        </Flex>

        {hasAccess('account', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/accounts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
