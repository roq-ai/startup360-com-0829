...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'service_provider.create': {
    roles: ['business-owner', 'team-member', 'administrator'],
    key: 'new-service-provider',
    tenantPath: ['platform', 'service_provider'],
    userPath: [],
  },
  'service_provider.update': {
    roles: ['business-owner', 'team-member', 'administrator'],
    key: 'service-provider-updated',
    tenantPath: ['platform', 'service_provider'],
    userPath: [],
  },
  'service_provider.delete': {
    roles: ['business-owner', 'team-member', 'administrator'],
    key: 'service-provider-removed',
    tenantPath: ['platform', 'service_provider'],
    userPath: [],
  },
};
...
const tenantRoles: string[] = ['business-owner', 'team-member', 'administrator'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.platform.findFirst({
