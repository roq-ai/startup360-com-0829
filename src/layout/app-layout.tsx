...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'team-member', 'administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PlatformInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/platforms` : null),
    () => getPlatforms({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Platforms',
      path: isTenantUser ? `/platforms/view/${data?.[0]?.id}` : '/platforms',
      entity: 'platform',
      service: AccessServiceEnum.PROJECT,
      icon: FiLayout,
    },
    { name: 'Accounts', path: '/accounts', entity: 'account', service: AccessServiceEnum.PROJECT, icon: FiCreditCard },
    {
      name: 'Preferred Service Providers',
      path: '/preferred-service-providers',
      entity: 'preferred_service_provider',
      service: AccessServiceEnum.PROJECT,
      icon: FiStar,
    },
    { name: 'Profiles', path: '/profiles', entity: 'profile', service: AccessServiceEnum.PROJECT, icon: FiInfo },
    {
